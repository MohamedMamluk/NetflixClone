...
import { MovieDetails, TVShow } from '../../types'
...
const Header: React.FC<HeaderProps> = ({ data }) => {
    ...
    useEffect(() => {
        let ratingAfter = Math.floor(data.getTVShowDetails.vote_average / 2)
        const array = []
        while (ratingAfter > 0) {
    ...
    return (
        <div
            ...
            style={{
                backgroundImage: `url(${data.getTVShowDetails.backdrop_path.original})`,
            ...
            <div className="relative z-20 flex flex-col w-full justify-between p-5 h-full text-center mb-4">
                ...
                <div className="w-full flex justify-between text-3xl">
                    ...
                    <div className="cursor-pointer">
                        {favoriteArray.find(
                            (item) => item == String(data.getTVShowDetails.id)
                        ) ? (
                            <AiFillHeart
                                fill="red"
                                size={30}
                                onClick={() =>
                                    removeFromFav(
                                        String(data.getTVShowDetails.id)
                                    )
                                }
                            />
                        ) : (
                            <MdFavoriteBorder
                                size={30}
                                onClick={() =>
                                    addToFav(String(data.getTVShowDetails.id))
                                }
                            />
                        )}
                    </div>
